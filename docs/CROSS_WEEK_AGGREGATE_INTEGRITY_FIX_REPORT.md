...
## Follow-up: repairing the already-stale 2026-09-07–13 aggregate

This fix corrects the code going forward but does not retroactively repair an aggregate already persisted before it existed. No HTTP route can trigger `reconcileWeekProgram` for a past week (`PUT /week/days/:day/activity` and the AI reconcile-week endpoint both reject a past target date), so repairing 2026-09-07–13's own stale aggregate — needed so 2026-09-14's carryover reads the corrected backlog — required a one-off, direct invocation of `computeFreshWeek`+`reconcileWeekProgram` against that specific past week, run only after explicit approval and isolated-copy verification. See `docs/logs/2026-09-14-56-sept-7-13-aggregate-repair-and-week-regeneration.md` for the full record, including the exact question asked and answered before production was touched.
