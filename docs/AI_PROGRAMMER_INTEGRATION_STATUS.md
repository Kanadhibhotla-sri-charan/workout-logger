...
**Generation alone still never persists a program.** Persistence, review, and commit are a separate, later, explicit step — see `docs/AI_PROGRAMMER_PHASE_2_PROPOSAL_LIFECYCLE.md` for the proposal storage/approval/commit workflow added on top of this vertical slice.
...
`proposal.proposalId` is always generated by the application (a fresh UUID per request) — a `proposalId` the model itself returns is discarded, never trusted or echoed back. `proposalId` (top-level) and `proposal.proposalId` are always the exact same string.

**Phase 2:** a successfully validated proposal IS now persisted (in `pending` status) — see `docs/AI_PROGRAMMER_PHASE_2_PROPOSAL_LIFECYCLE.md` for the full storage/review/commit workflow, the new `GET/POST /api/ai-programmer/proposals/:proposalId*` endpoints, and why generation alone still never touches `workout_sessions`/`program_sessions`. Re-requesting the same still-editable date creates a separate new proposal row each time (no automatic deduplication) — this endpoint's own behavior is otherwise unchanged from the first vertical slice.
