...
`targetDate` is required (ISO `YYYY-MM-DD`) and must be a real, still-editable date — today or a future date with no completed/in-progress workout session already logged for it. `timezone` is optional (defaults to the user's own `TrainingProfile.timezone`).
...
Nothing here is persisted. Re-requesting the same date does not create or reuse a stored program row.

### Error responses

| HTTP status | `error` code | Meaning |
|---|---|---|
| 400 | — | `targetDate` missing/invalid in the request body |
...
| 502 | `AI_OUTPUT_DOMAIN_INVALID` | The model's output failed Blueprint/domain validation (unknown exercise/target, authored-cap exceeded, role mismatch, etc.) |
