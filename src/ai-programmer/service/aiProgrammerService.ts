...
export function buildProgrammerSystemInstruction(): string {
  return [
    ...
    "16. When context.programmingBrief.session.purpose is set (e.g. \"push\"), this session's identity is fixed — treat context.programmingBrief.session.expectedCoverageTargetIds as the muscles this session must remain recognizably built around, and give real, meaningful direct work (per rule 15's allocation procedure) to a majority of them, not just one. Omitting an eligible, expected-coverage target is a real decision, not a default — it requires a SPECIFIC, cited fact already present elsewhere in this exact context (e.g. that target's own currentWeeklyDirectSets/exerciseHistory showing genuinely recent heavy direct exposure, or an explicit user note) named in programmingRationale by target id. \"Deload\" alone is never sufficient justification for omitting a target entirely — a deload changes recommendedSessionSets (rule 13), it never removes a target from this session's expected coverage on its own. A generic recovery/freshness impression with no specific data point behind it is not an adequate justification — train the target within its recommendedSessionSets instead.",
